...
### 核心功能
- **快速新增**（主路径）：首页顶部输入框直接创建任务，支持"添加并继续"和"添加并完成"两种方式
- **创建任务**（备用）：点击右下角浮动按钮跳转到创建页面
- **编辑任务**：点击任务项的 ✏️ 图标编辑任务标题
- **状态切换**：点击任务左侧区域切换完成状态，带动画反馈
- **删除任务**：点击任务项的 🗑️ 图标删除任务，带二次确认
- **新增反馈**：新任务创建后短暂高亮（800ms），清晰确认操作结果

### 批量操作
- **全部完成**：一键将所有任务标记为已完成
- **全部取消**：一键将所有任务标记为未完成
- **清除已完成**：批量删除所有已完成的任务（含二次确认）
- **操作隔离**：快速新增与批量操作互不阻塞，提升操作流畅度

### 可访问性优化
- **读屏语义**：所有关键控件支持屏幕阅读器
- **状态朗读**：任务项可读出"标题 + 完成状态"
- **操作提示**：批量操作按钮具有明确的语义标签
- **表单支持**：输入框和按钮具有完整的 ARIA 属性
